...
const fetchChat = async (req, res, next) => {
  try {
    Chat.find({
      ...
      .populate("users", "-password")
      .populate("isGroupAdmin latestMessage", "-password")
      .sort({ updatedAt: -1 })
      .then(async (results) => {
...
const createGroup = async (req, res, next) => {
  ...
  if (users.length < 2) {
    return res
      .status(400)
      .send({ message: "groups must be more than 2 members" });
  }
  ...
  try {
    ...
    fullGroupChat = await Chat.findOne({ _id: groupChat._id })
      .populate("users", "-password")
...
const deletechat = async (req, res, next) => {
  ...
  removed = await Chat.findOneAndDelete({ _id: chatId });
